	...
	$('#files').change(function(){
		...
		for(var i = 0 ; i<files.length ; i++)
			...
			if(error == '' && cc <= 6 && files.length > 0)
					...
					for(var count = 0 ; count<files.length ; count++)
						{
							...
							for (var i =0 ; i < cl; i++) 
							{
								loader += '<div class="photo"><img src="../../public/images/loader.gif" style="width: 70px; margin: 10px auto; display: block;"></div>';
							}
							$('#loader').html(loader);
						    ...
						    success(result) {
						    	...
						    	form_data.append('files[]',	result, result.name); 
									...
									$.ajax({
									url:			"http://localhost/index.php/annonce/do_upload",
									method:			"post",
									data:			form_data,
									contentType:	false,
									cache:			false,
				...
				else
				{
					if(cc>6) 
					{
						alert('Seulement 6 photos sont autorisées par annonce!');
					}
					else if(error =! '' && error) 
						{
							alert(error);
		...
		$(document).on ("click", ".remove", function () {
			...
			var b = false;
				...
				$.ajax({
					url:			"http://localhost/index.php/annonce/delete_img/"+href,
					method:			"post",
					contentType:	false,
					cache:			false,
					processData:	false,
